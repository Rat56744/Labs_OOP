...
using std::cout, std::vector, std::setw, std::left, std::setfill, std::endl;
...
void baze_data(vector<std::shared_ptr<common>>& people, vector<int>& max_stud, vector<int>& max_prep)
{
    ...
    cout<<setw(max_prep[4]+3)<<left<<"Academical course"<<endl;
    cout<<setfill('-')<<setw(max_prep[0]+max_prep[1]+max_prep[2]+max_prep[3]+max_prep[4]+15)<<'-'<<setfill(' ')<<endl;
